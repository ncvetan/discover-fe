...
function PlaceCard(props) {
    ...
    return (
        <>
            ...
            >
                <div className="mt-8 h-auto w-auto flex flex-col m-2 p-2 items-center bg-primary-green rounded-2xl shadow-md text-white">
                    <h1 className="text-xl pb-1">{data.name}</h1>
                    <div
                        style={{
                    ...
                    />
                    <p className="pt-1">{`${data.address.streetNumber} ${
                        data.address.streetName
                    } ${data.address.unit ? `#${data.address.unit}` : ''}`}</p>
                    <p className="pt-1">{`${data.address.city}, ${data.address.postalCode} ${data.address.countryCode}`}</p>
